...
sena_enquete <- function(don,n, propi, propt){
 vect_mu <- 1:50
 for(i in 1:50){
   d <- don[sample(nrow(don),n),]
   d <- d |> mutate(Z = as.numeric(Pi <= quantile(don$Pi, probs = propi))) |> 
     mutate(W = as.numeric(Pi >= quantile(don$Pi, probs = propi) & 
                              Pt <= quantile(don$Pt, probs = propt))) 
 }
 # d$Y[d$Z==0 &d$W==0] <- NA
 # d$X1[d$Z==0 &d$W==0] <- NA
 return(d)
...
$$\begin{pmatrix} 
        & X_0 & X_1 & U & P_i & P_t \\
    \hline
        X_0 & 1&0.3&0.2&0.1&0.1 \\
        X_1 & 0.3&1&0.4&0.2&0.1  \\
        U & 0.2&0.4&1&0.8&0.7  \\
        P_i & 0.1&0.2&0.8&1&0.4  \\
        P_t & 0.1&0.1&0.7&0.4&1 
   \end{pmatrix}$$
...
vec_mu <- 1:50
for(i in 1:50){ 
  d <- sena_enquete(don, 10000, 0.3, 0.5) 
  reg <- glm(formula = Z+W~X0, data = d, family = binomial)
  
  vec_mu[i] <- (1/10^6)*sum(d$Y*(d$Z+d$W)/(pii*reg$fitted.values), na.rm = TRUE)
}
```
...
## simulation des données de la population avec une faible influence de U sur Y, Pi et Pt
