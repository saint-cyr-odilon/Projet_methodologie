...
```{r echo=FALSe, message=FALSE, warning=FALSE}
don1 |> pivot_longer(cols = names(don), names_to = "Variables", values_to = "Valeurs") |> 
...
for(i in 1:500){
  ...
  reg <- glm(formula = Z+W~X0+X1+Y, data = d[d$Z==0,], family = binomial)
  p_chap[d$Z==0] <- reg$fitted.values
